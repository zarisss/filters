...
class IMUSimulator:
    ...
    def simulate(self):
        g = 9.81
        time = np.linspace(0, self.T, int(self.T / self.dt)) #generating time intervals
        angle_in_deg = self.amplitude * np.sin(2 * np.pi * self.freq * time) #angular displacement
        angle_in_rad = np.radians(angle_in_deg)
        true_omega = np.gradient(angle_in_rad, self.dt) #finding angular velocity
        drift = self.gyro_drift * np.linspace(0, 1, len(time)) # adding drift error
        noise = np.random.normal(0, self.gyro_noise_std_deviation, len(time)) #adding gaussian noise
        gyro_measured = true_omega + drift + noise
        
        #acceleration in z component
